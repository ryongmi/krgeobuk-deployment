// authz-server CI/CD 파이프라인 Job 정의
// 수동 실행: Build with Parameters → ENVIRONMENT, GIT_BRANCH 선택

def githubOrg = System.getenv('GITHUB_ORG') ?: 'your-org'

pipelineJob('authz-server-pipeline') {
    description('authz-server CI/CD 파이프라인 (권한 관리 서버)')
    ...
    definition {
        cpsScm {
            ...
            scriptPath('jenkins/Jenkinsfile.authz-server')
        }
    }
}
