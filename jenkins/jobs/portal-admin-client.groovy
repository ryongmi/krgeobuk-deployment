// portal-admin-client CI/CD 파이프라인 Job 정의
// 수동 실행: Build with Parameters → ENVIRONMENT, GIT_BRANCH 선택

def githubOrg = System.getenv('GITHUB_ORG') ?: 'your-org'

pipelineJob('portal-admin-client-pipeline') {
    description('portal-admin-client CI/CD 파이프라인 (포털 관리자 클라이언트)')
    ...
    definition {
        cpsScm {
            ...
            scriptPath('jenkins/Jenkinsfile.portal-admin-client')
        }
    }
}
